...
uniform float lightintensity; //abmient
...
void main() {
  ...
  float k_a = 1.0; // says how much of ambient light is reflected
  float k_d = 0.9; // diffuse reflection coefficient
  float k_s = 0.5; // specular reflection coefficient
  float shininess = 16.0; 

  vec3 ambient =  k_a * lightintensity * lightcolor;
  vec3 diffuse =  k_d * lightintensity * (max(dot(L,N),0.0)) * lightcolor;
  vec3 specular = k_s * lightintensity * pow(max(dot(halfway_direction, N), 0.0), shininess) * specularcolor ;

  vec3 blinn_phong_color = (ambient + diffuse) * planetcolor + specular;
